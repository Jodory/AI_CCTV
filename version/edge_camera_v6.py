...
def display_original(timeData, shared_array, shape):
    ...
    while True:
        ...
        cv2.imwrite(f'camera/original/{{''.join(timeData.value.decode('utf-8'))}}.jpg', frame1)
...
def display_process(timeData, shared_array, shape):
    ...
    while True:
        ...
        while True:
            ...
            print(f'process_timeData: {''.join(timeData.value.decode('utf-8'))}')
            cv2.imwrite(f'camera/process/{{''.join(timeData.value.decode('utf-8'))}}.jpg', frame)
            break
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
    cv2.destroyAllWindows()
...
def main():
    ...
    # Create a shared string to store timeData. We'll assume a maximum length of 50 for the datetime string.
    timeData = mp.Array('c', 50)
    ...
    while True:
        ...
        if ret:
            ...
            # Update the shared timeData
            current_time = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
            timeData.value = current_time.encode('utf-8')
