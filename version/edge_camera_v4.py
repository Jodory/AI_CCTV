...
def display_original(timeData, shared_array, shape):
    ...
    while True:
        ...
        # global timeData
        timeData.value = time.mktime(datetime.now().timetuple()).datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        cv2.imwrite(f'camera/original/{timeData.value}.jpg', frame1)
...
def display_process(timeData, shared_array, shape):
    ...
    while True:
        ...
        while True:
            ...
            # global timeData
            timeData.value = time.mktime(datetime.now().timetuple()).datetime.now().strftime("%Y%m%d_%H%M%S_%f")
            cv2.imwrite(f'camera/process/{timeData.value}.jpg', frame)
            break
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
    cv2.destroyAllWindows()
