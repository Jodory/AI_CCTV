...
is_display_process_running = mp.Value('b', False)
import onnx
import numpy as np
import time
from cv2 import *  
...
def display_original(timeData, shared_array, shape):
    ...
    is_display_process_running.value = False

def display_process(timeData, shared_array, shape):
    global is_display_process_running
    is_display_process_running.value = True
    print(f'process2: 시작')
...
def main():
    ...
    timeData.value = datetime.now().strftime('%Y%m%d_%H%M%S_%f').encode('utf-8')
    # print(timeData.value)
    ...
    if not is_display_process_running.value:
        process2 = mp.Process(target=display_process, args=(timeData, shared_array, shape))
    else:
        print("display_process is already running!")
